...
def compute_shap_bar(model, X_sample: pd.DataFrame, top_n=15):
    """
    SHAP из CatBoost: get_feature_importance(type="ShapValues") требует Pool.
    Возвращает mean(|shap|) по признакам и bar chart.
    """
    # на всякий случай убираем NaN и приводим к числам
    X_sample = X_sample.copy()
    X_sample = X_sample.replace([np.inf, -np.inf], np.nan).fillna(0)

    pool = Pool(X_sample)  # <-- ключевое исправление

    shap_vals = model.get_feature_importance(type="ShapValues", data=pool)
    # shap_vals: (n_samples, n_features + 1), последний столбец — base value
